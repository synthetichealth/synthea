...
public final class LifecycleModule extends Module {
  ...
  private static void diabeticVitalSigns(Person person, long time) {
    ...
    /*
     * blood_pressure: normal: systolic: [100,139] # mmHg diastolic: [70,89] # mmHg hypertensive:
     * systolic: [140,200] # mmHg diastolic: [90,120] # mmHg
     */
    if (hypertension) {
  ...
  private static void chanceOfLungCancer(Person person, long time) {
    ...
    /*
     * Overall, the chance that a man will develop lung cancer in his lifetime is about 1 in 14; for
     * a woman, the risk is about 1 in 17.
     * http://www.cancer.org/cancer/lungcancer-non-smallcell/detailedguide/non-small-cell-lung-
     * cancer-key-statistics Men who smoke are 23 times more likely to develop lung cancer. Women
     * are 13 times more likely, compared to never smokers.
     * http://www.lung.org/lung-health-and-diseases/lung-disease-lookup/lung-cancer/learn-about-lung
     * -cancer/lung-cancer-fact-sheet.html In a 2006 European study, the risk of developing lung
     * cancer was: 0.2 percent for men who never smoked (0.4% for women); 5.5 percent of male former
     * smokers (2.6% in women); 15.9 percent of current male smokers (9.5% for women); 24.4 percent
     * for male "heavy smokers" defined as smoking more than 5 cigarettes per day (18.5 percent for
     * women) https://www.verywell.com/what-percentage-of-smokers-get-lung-cancer-2248868
     */
  ...
  private static void startAlcoholism(Person person, long time) {
    ...
    if (person.attributes.get(Person.ALCOHOLIC) == null && person.ageInYears(time) == 25) {
      Boolean alcoholic = person.rand() < 0.025; // TODO assume about 8 mil alcoholics/320 mil gen
                                                 // pop
