...
//      }
    ...
    private static Entry basicInfo(Person person, Bundle bundle, long stopTime) {
        ...
        patientResource.getMeta().addProfile("http://fhir.org/guides/argonaut/StructureDefinition/argo-patient");
        ...
        raceExtension.setUrl("http://fhir.org/guides/argonaut/StructureDefinition/argo-race");
        String race = (String) person.attributes.get(Person.RACE);

        ExtensionDt ethnicityExtension = new ExtensionDt();
        ethnicityExtension.setUrl("http://fhir.org/guides/argonaut/StructureDefinition/argo-ethnicity");
        ...
        Code raceCode = new Code(
                "http://fhir.org/guides/argonaut/v3/Race",
                (String) raceEthnicityCodes.get(race),
                raceDisplay);

        Code ethnicityCode = new Code(
                "http://fhir.org/guides/argonaut/v3/Ethnicity",
...
//    ethnicityExtension.setValue(mapCodeToCodeableConcept(ethnicityCode, "http://hl7.org/fhir/v3/Ethnicity"));
        final ExtensionDt txtExtensionDt = new ExtensionDt();
        txtExtensionDt.setUrl("text");
        txtExtensionDt.setValue(new StringDt("Mixed"));

        final ExtensionDt raceExtensionDetails = new ExtensionDt();
        raceExtensionDetails.setUrl("ombCategory");
        raceExtensionDetails.setValue(mapCodeToCodingDt(raceCode, "http://fhir.org/guides/argonaut/v3/Race"));
        raceExtension.addUndeclaredExtension(raceExtensionDetails);
        raceExtension.addUndeclaredExtension(txtExtensionDt);

        final ExtensionDt ethnicityExtensionDetails = new ExtensionDt();
        ethnicityExtensionDetails.setUrl("ombCategory");
        ethnicityExtensionDetails.setValue(mapCodeToCodingDt(raceCode, "http://fhir.org/guides/argonaut/v3/Ethnicity"));
        ethnicityExtension.addUndeclaredExtension(ethnicityExtensionDetails);
        ethnicityExtension.addUndeclaredExtension(txtExtensionDt);
        ...
        birthSexExtension.setUrl("http://fhir.org/guides/argonaut/StructureDefinition/argo-birthsex");
...
//                        .setServiceProvider(new ResourceReferenceDt(providerOrganization.getFullUrl()));
    ...
    /**
     * Helper function to convert a Code into a CodingDt. Takes an optional system, which
     * replaces the Code.system in the resulting CodingDt if not null.
     *
     * @param from   The Code to create a CodingDt from.
     * @param system The system identifier, such as a URI. Optional; may be null.
     * @return The converted CodingDt
     */
    private static CodingDt mapCodeToCodingDt(Code from, String system) {
        CodingDt coding = new CodingDt();
        if (from.display != null) {
            coding.setDisplay(from.display);
