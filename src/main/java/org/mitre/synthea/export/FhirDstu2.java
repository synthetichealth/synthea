...
public class FhirDstu2 {
	...
	private static Entry basicInfo(Person person, Bundle bundle, long stopTime) {
		...
		patientResource.addIdentifier().setType(IdentifierTypeCodesEnum.MR)
				.setSystem("http://hospital.smarthealthit.org").setValue((String) person.attributes.get(Person.ID));

		patientResource.addIdentifier().setType(IdentifierTypeCodesEnum.SOCIAL_BENEFICIARY_IDENTIFIER)
				.setSystem("http://hl7.org/fhir/sid/us-ssn")
				.setValue((String) person.attributes.get(Person.IDENTIFIER_SSN));

		if (person.attributes.get(Person.IDENTIFIER_DRIVERS) != null) {
			patientResource.addIdentifier().setType(IdentifierTypeCodesEnum.DL)
		...
		Code ethnicityCode = new Code("http://hl7.org/fhir/v3/Ethnicity", (String) raceEthnicityCodes.get(ethnicity),
				ethnicityDisplay);
		...
		patientResource.addTelecom().setSystem(ContactPointSystemEnum.PHONE).setUse(ContactPointUseEnum.HOME)
	 ...
	 * given Bundle.
	 *
	 * @param person      Patient at the encounter.
	 ...
	 * @return the added Entry
	 */
	private static Entry medicationClaim(Entry personEntry, Bundle bundle, Entry encounterEntry, Claim claim,
			Entry medicationEntry) {
		ca.uhn.fhir.model.dstu2.resource.Claim claimResource = new ca.uhn.fhir.model.dstu2.resource.Claim();
		ca.uhn.fhir.model.dstu2.resource.Encounter encounterResource = (ca.uhn.fhir.model.dstu2.resource.Encounter) encounterEntry
				.getResource();
	 ...
	 * @return the added Entry
	 */
	private static Entry encounterClaim(Entry personEntry, Bundle bundle, Entry encounterEntry, Claim claim) {
		ca.uhn.fhir.model.dstu2.resource.Claim claimResource = new ca.uhn.fhir.model.dstu2.resource.Claim();
		ca.uhn.fhir.model.dstu2.resource.Encounter encounterResource = (ca.uhn.fhir.model.dstu2.resource.Encounter) encounterEntry
				.getResource();
		...
		ca.uhn.fhir.model.dstu2.resource.Encounter encounter = (ca.uhn.fhir.model.dstu2.resource.Encounter) encounterEntry
				.getResource();
		...
		for (HealthRecord.Entry item : claim.items) {
			if (Costs.hasCost(item)) {
				...
				itemType.setSystem("http://hl7.org/fhir/v3/ActCode").setCode("CSINV")
	 ...
	 * @return The added Entry
	 */
	private static Entry allergy(Entry personEntry, Bundle bundle, Entry encounterEntry, HealthRecord.Entry allergy) {
	 ...
	 * @return The added Entry
	 */
	private static Entry observation(Entry personEntry, Bundle bundle, Entry encounterEntry, Observation observation) {
	 ...
	 * @return The added Entry
	 */
	private static Entry procedure(Entry personEntry, Bundle bundle, Entry encounterEntry, Procedure procedure) {
		...
		if (procedure.stop != 0L) {
			...
			procedureResource
					.setPerformed(new PeriodDt().setStart(new DateTimeDt(startDate)).setEnd(new DateTimeDt(endDate)));
		} else {
	 ...
	 * @return The added Entry
	 */
	private static Entry medication(Entry personEntry, Bundle bundle, Entry encounterEntry, Medication medication) {
	 ...
	 * @return The added Entry
	 */
	private static Entry medicationAdministration(Entry personEntry, Bundle bundle, Entry encounterEntry,
	 ...
	 * @return The added Entry
	 */
	private static Entry report(Entry personEntry, Bundle bundle, Entry encounterEntry, Report report) {
		DiagnosticReport reportResource = new DiagnosticReport();
		reportResource.setStatus(DiagnosticReportStatusEnum.FINAL);
		/*
		 * Technically, the CodeableConcept system should be
		 * "http://hl7.org/fhir/v2/0074" But the official Argonauts profiles incorrectly
		 * list the category pattern as the ValueSet (which contains the above system)
		 * as "http://hl7.org/fhir/ValueSet/diagnostic-service-sections", so we repeat
		 * the error here.
		 */
		CodeableConceptDt category = new CodeableConceptDt("http://hl7.org/fhir/ValueSet/diagnostic-service-sections",
				"LAB");
	 ...
	 * @return The added Entry
	 */
	private static Entry careplan(Entry personEntry, Bundle bundle, Entry encounterEntry, CarePlan carePlan) {
	...
	/**
	 * Map the given Media element to a FHIR Media resource, and add it to the given
	 * Bundle.
	 *
	 * @param personEntry    The Entry for the Person
	 ...
	 * @return The added Entry
	 */
	private static Entry media(Entry personEntry, Bundle bundle, Entry encounterEntry, Observation obs) {
	 ...
	 * @return The added Entry.
	 */
	private static Entry device(Entry personEntry, Bundle bundle, HealthRecord.Device device) {
	 ...
	 * @return The added Entry.
	 */
	private static Entry supplyDelivery(Entry personEntry, Bundle bundle, HealthRecord.Supply supply,
			Encounter encounter) {
		...
		// super hackish -- there's no "code" field available here, just a reference to
		// a Device
	...
	protected static Entry provider(Bundle bundle, Provider provider) {
		...
		AddressDt address = new AddressDt().addLine(provider.address).setCity(provider.city).setPostalCode(provider.zip)
		...
		if (provider.phone != null && !provider.phone.isEmpty()) {
			ContactPointDt contactPoint = new ContactPointDt().setSystem(ContactPointSystemEnum.PHONE)
	...
	protected static Entry practitioner(Bundle bundle, Clinician clinician) {
		...
		practitionerResource.getName().addFamily((String) clinician.attributes.get(Clinician.LAST_NAME))
		...
		AddressDt address = new AddressDt().addLine((String) clinician.attributes.get(Clinician.ADDRESS))
	 ...
	 * @param time     If true, return a DateTimeDt; if false, return a DateDt.
	...
	/**
	 * Helper function to convert a Code into a CodeableConceptDt. Takes an optional
	 * system, which replaces the Code.system in the resulting CodeableConceptDt if
	 * not null.
	 *
