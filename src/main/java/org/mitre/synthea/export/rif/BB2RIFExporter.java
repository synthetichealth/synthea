...
import org.mitre.synthea.export.rif.BB2RIFStructure.HHA;
import org.mitre.synthea.export.rif.BB2RIFStructure.HOSPICE;
import org.mitre.synthea.export.rif.BB2RIFStructure.INPATIENT;
import org.mitre.synthea.export.rif.BB2RIFStructure.NPI;
import org.mitre.synthea.export.rif.BB2RIFStructure.OUTPATIENT;
import org.mitre.synthea.export.rif.BB2RIFStructure.PDE;
import org.mitre.synthea.export.rif.ExporterBase.ClaimType;
...
public class BB2RIFExporter {
  ...
  final long claimCutoff;
  final long snfPDPMCutover;

  final List<LinkedHashMap<String, String>> carrierLookup;
  ...
  public void exportEndState() throws IOException {
    ...
    endState.setProperty("exporter.bfd.bene_id_start", ExporterBase.nextBeneId.toString());
    endState.setProperty("exporter.bfd.clm_id_start", ExporterBase.nextClaimId.toString());
    endState.setProperty("exporter.bfd.clm_grp_id_start", ExporterBase.nextClaimGroupId.toString());
  ...
  public void export(Person person, long stopTime, int yearsOfHistory) throws IOException {
    ...
    exportCounts.put(EXPORT_SUMMARY.BENE_ID, beneExp.exportBeneficiary(person));
    beneExp.exportBeneficiaryHistory(person);
   ...
   */
  List<String> getDiagnosesCodes(Person person, long time) {
   ...
   */
  boolean hasESRD(Person person, int year) {
  ...
  static boolean isBlind(Person person) {
  ...
  private long exportOutpatient(Person person, long startTime, long stopTime)
    ...
    for (Encounter encounter : person.record.encounters) {
      ...
      if (!ExporterBase.getClaimTypes(encounter).contains(ClaimType.OUTPATIENT)) {
        continue;
      }
      ...
      // The REQUIRED fields
      fieldValues.put(OUTPATIENT.BENE_ID, (String) person.attributes.get(ExporterBase.BB2_BENE_ID));
      ...
      fieldValues.put(OUTPATIENT.CLM_FROM_DT, ExporterBase.bb2DateFromTimestamp(encounter.start));
      fieldValues.put(OUTPATIENT.CLM_THRU_DT, ExporterBase.bb2DateFromTimestamp(encounter.stop));
  ...
  private long exportInpatient(Person person, long startTime, long stopTime)
    ...
    for (Encounter encounter : person.record.encounters) {
      ...
      if (!ExporterBase.getClaimTypes(encounter).contains(ClaimType.INPATIENT)) {
      ...
      // The REQUIRED fields
      fieldValues.put(INPATIENT.BENE_ID, (String) person.attributes.get(ExporterBase.BB2_BENE_ID));
  ...
  private long exportCarrier(Person person, long startTime, long stopTime) throws IOException {
    ...
    for (Encounter encounter : person.record.encounters) {
      ...
      if (!ExporterBase.getClaimTypes(encounter).contains(ClaimType.CARRIER)) {
        continue;
      }

      long claimId = ExporterBase.nextClaimId.getAndDecrement();
      long claimGroupId = ExporterBase.nextClaimGroupId.getAndDecrement();
      long carrClmId = ExporterBase.nextCarrClmCntlNum.getAndDecrement();
      ...
      fieldValues.put(CARRIER.BENE_ID, (String) person.attributes.get(ExporterBase.BB2_BENE_ID));
  ...
  private long exportPrescription(Person person, long startTime, long stopTime)
    ...
    PartDContractHistory partDContracts =
            (PartDContractHistory) person.attributes.get(ExporterBase.BB2_PARTD_CONTRACTS);
    ...
    for (Encounter encounter : person.record.encounters) {
      ...
      if (ExporterBase.isVAorIHS(encounter)) {
        continue;
      }
    ...
    for (PrescriptionFill fill: prescriptionFills) {
      ...
      long pdeId = ExporterBase.nextPdeId.getAndDecrement();
      long claimGroupId = ExporterBase.nextClaimGroupId.getAndDecrement();
      ...
      fieldValues.put(PDE.BENE_ID, (String) person.attributes.get(ExporterBase.BB2_BENE_ID));
      fieldValues.put(PDE.SRVC_DT, ExporterBase.bb2DateFromTimestamp(fill.time));
      ...
      fieldValues.put(PDE.PD_DT, ExporterBase.bb2DateFromTimestamp(fill.time));
  ...
  private long exportDME(Person person, long startTime, long stopTime)
    ...
    for (Encounter encounter : person.record.encounters) {
      ...
      fieldValues.put(DME.BENE_ID, (String) person.attributes.get(ExporterBase.BB2_BENE_ID));
      ...
      fieldValues.put(DME.LINE_LAST_EXPNS_DT, ExporterBase.bb2DateFromTimestamp(encounter.stop));
      ...
      synchronized (rifWriters.getOrCreateWriter(DME.class)) {
        ...
        for (ClaimEntry lineItem : encounter.claim.items) {
          ...
          fieldValues.put(DME.CLM_THRU_DT, ExporterBase.bb2DateFromTimestamp(lineItem.entry.start));
  ...
  private long exportHome(Person person, long startTime, long stopTime) throws IOException {
    ...
    for (Encounter encounter : person.record.encounters) {
      ...
      if (!ExporterBase.getClaimTypes(encounter).contains(ClaimType.HHA)) {
        continue;
      }
      List<String> mappedDiagnosisCodes = getDiagnosesCodes(person, encounter.stop);
      if (mappedDiagnosisCodes.isEmpty()) {
    ...
    for (ConsolidatedServicePeriod servicePeriod: servicePeriods.getPeriods()) {
      ...
      // The REQUIRED fields
      fieldValues.put(HHA.BENE_ID,  (String) person.attributes.get(ExporterBase.BB2_BENE_ID));
      ...
      Encounter finalEncounterOfPeriod = servicePeriod.getEncounters().get(
      ...
      if (finalEncounterOfPeriod.reason != null) {
        // If the encounter has a recorded reason, enter the mapped
        // values into the principle diagnoses code.
        if (conditionCodeMapper.canMap(finalEncounterOfPeriod.reason.code)) {
          String icdCode = conditionCodeMapper.map(finalEncounterOfPeriod.reason.code, person,
                  true);
      ...
      // Use the active condition diagnoses to enter mapped values
      // into the diagnoses codes.
      List<String> mappedDiagnosisCodes = getDiagnosesCodes(person, finalEncounterOfPeriod.stop);
      int smallest = Math.min(mappedDiagnosisCodes.size(),
      ...
      synchronized (rifWriters.getOrCreateWriter(HHA.class)) {
        ...
        for (ConsolidatedClaimLines.ConsolidatedClaimLine lineItem:
          ...
          fieldValues.put(HHA.REV_CNTR_DT, ExporterBase.bb2DateFromTimestamp(lineItem.getStart()));
        ...
        fieldValues.remove(HHA.REV_CNTR_RATE_AMT);
        fieldValues.remove(HHA.REV_CNTR_UNIT_CNT);
        fieldValues.remove(HHA.REV_CNTR_DDCTBL_COINSRNC_CD);
        rifWriters.writeValues(HHA.class, fieldValues);
      }
      claimCount++;
    }
  ...
  private static void setHHAClaimLineCosts(HashMap<HHA, String> fieldValues, ClaimCost lineItem,
          int count) {
  ...
  private long exportHospice(Person person, long startTime, long stopTime) throws IOException {
    ...
    for (Encounter encounter : person.record.encounters) {
      ...
      if (!ExporterBase.getClaimTypes(encounter).contains(ClaimType.HOSPICE)) {
        continue;
      }
      ...
      fieldValues.put(HOSPICE.BENE_ID, (String) person.attributes.get(ExporterBase.BB2_BENE_ID));
      ...
      if (encounter.ended) {
        ...
        patientStatus = "A"; // discharged
        dischargeDate = ExporterBase.bb2DateFromTimestamp(ExportHelper.nextFriday(encounter.stop));
      } else {
      ...
      synchronized (rifWriters.getOrCreateWriter(HOSPICE.class)) {
        ...
        if (claimLine == 1) {
          ...
          // Add a single top-level entry.
          fieldValues.put(HOSPICE.CLM_LINE_NUM, Integer.toString(claimLine));
          fieldValues.put(HOSPICE.REV_CNTR_DT, ExporterBase.bb2DateFromTimestamp(encounter.start));
