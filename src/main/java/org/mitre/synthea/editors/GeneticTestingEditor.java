...
import org.hl7.fhir.dstu3.model.codesystems.LOINC480020Answerlist;
...
public class GeneticTestingEditor extends StatefulHealthRecordEditor {
  ...
  private static final String PRIOR_GENETIC_TESTING = "PRIOR_GENETIC_TESTING";
  private static final String GENETIC_TESTING_REPORT_TYPE = "Genetic analysis summary panel";
  private static final double GENETIC_TESTING_THRESHOLD = 0.80;
  private static final String TRIGGER_CONDITIONS[] = {
    "stroke",
  ...
  public boolean shouldRun(Person person, HealthRecord record, long time) {
    ...
    return person.rand() >= GENETIC_TESTING_THRESHOLD;
  }
...
//    HealthRecord.Observation observation = person.record.new Observation(time, GENETIC_TESTING_REPORT_TYPE, null);
//    observation.codes.add(new Code("LOINC", "55232-3", GENETIC_TESTING_REPORT_TYPE));
//    observations.add(observation);
    HealthRecord.Report geneticTestingReport = person.record.new Report(time, GENETIC_TESTING_REPORT_TYPE, observations);
    geneticTestingReport.codes.add(new Code("LOINC", "55232-3", GENETIC_TESTING_REPORT_TYPE));
    encounter.reports.add(geneticTestingReport);
    Map<String, Object> context = this.getOrInitContextFor(person);
    context.put(PRIOR_GENETIC_TESTING, time);
    System.out.println("Added genetic testing report");
  }
