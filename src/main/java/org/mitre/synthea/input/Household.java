...
public class Household {
   ...
   * 
   * @param newAdult the adult to be added.
   */
  public void addAdult(Person newAdult) {
    if (newAdult == null) {
      ...
      // We're assuming that, adults in households are married. Prevents last names from changing.
      ...
      throw new RuntimeException(
          "There can only be a max of 2 adults per household and a 3rd was added. Household"
          + " already includes adults "
  ...
  /**
   * Adds a child to the dependents of the household.
   * 
  ...
  public boolean includesPerson(Person person) {
    FixedRecord seedRecord = ((FixedRecordGroup) person.attributes.get(Person.RECORD_GROUP))
        .seedRecord;
    boolean matchesFirstAdult = (this.firstAdult == null) ? false : ((FixedRecordGroup)
        this.firstAdult.attributes.get(Person.RECORD_GROUP)).seedRecord.equals(seedRecord);
    boolean matchesSecondAdult = (this.secondAdult == null) ? false : ((FixedRecordGroup)
