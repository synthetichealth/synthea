...
public class Graphviz 
{
	...
	private static void generateJsonModuleGraph(JsonObject module, File outputFolder, String relativePath) throws IOException
	{
		...
		for(Map.Entry<String, JsonElement> s : states.entrySet())
		{
			...
			{
				node = node.with(Color.BLACK.fill())
				 	.with(Style.ROUNDED.and(Style.FILLED))
		...
		for(Map.Entry<String, JsonElement> s : states.entrySet())
		{
			...
			if (state.has("direct_transition"))
			{
				...
				if (target == null)
				{
					throw new RuntimeException(relativePath + " " + name + " transitioning to unknown state: " + targetName);
				}
			...
			} else if (state.has("conditional_transition"))
			{
				...
				for (int i = 0 ; i < cts.size() ; i++)
				{
					...
					String cond = ct.has("condition") ? logicDetails(ct.get("condition").getAsJsonObject()) : "else";
			...
			} else if (state.has("distributed_transition"))
			{
				...
				distributions.forEach( d -> {
					...
					if (dx.isJsonObject()) {
						...
						label = "p("
								+ distribution.get("attribute").getAsString()
								+ ")";
					...
					if (target == null)
					{
						throw new RuntimeException(relativePath + " " + name + " transitioning to unknown state: " + destination);
					}
		...
		outputFile.mkdirs();
		guru.nidi.graphviz.engine.Graphviz.fromGraph(g).rasterizer(Rasterizer.SALAMANDER).render(Format.PNG).toFile(outputFile);
	}
	...
	private static String getStateDescription(JsonObject state) 
	{
		...
		switch (type) {
		case "Guard":
			details.append("Allow if "
					+ logicDetails(state.get("allow").getAsJsonObject()));
			break;
		case "Delay":
		case "Death":
			if (state.has("range")) {
				...
				details.append(low).append(" - ").append(high).append(" ")
						.append(unit);
		...
		case "EncounterEnd":
			...
			if (state.has("discharge_disposition")) {
				JsonObject coding = state.get("discharge_disposition")
						.getAsJsonObject();
				String code = coding.get("code").getAsString();
				String display = coding.get("display").getAsString();
				details.append("\\lDischarge Disposition: [").append(code)
						.append("] ").append(display);
			}
			break;
		case "SetAttribute":
			String v = state.has("value") ? state.get("value").getAsString()
					: null;
			details.append("Set ").append(state.get("attribute").getAsString())
					.append(" = ").append(v);
			break;
		case "Symptom":
			...
			if (state.has("range")) {
				...
				String low = r.get("low").getAsString();
				String high = r.get("high").getAsString();
				details.append(s).append(": ").append(low).append(" - ")
						.append(high);
			} else if (state.has("exact")) {
				JsonObject e = state.get("exact").getAsJsonObject();
				String quantity = e.get("quantity").getAsString();
		...
		case "Counter":
			...
			details.append(action).append(" value of attribute ")
					.append(attribute).append(" by 1");
			break;
		case "VitalSign":
			...
			if (state.has("range")) {
				...
				String low = r.get("low").getAsString();
				String high = r.get("high").getAsString();
				details.append("Set ").append(vs).append(": ").append(low)
						.append(" - ").append(high).append(" ").append(unit);
			} else if (state.has("exact")) {
				JsonObject e = state.get("exact").getAsJsonObject();
				String quantity = e.get("quantity").getAsString();
				details.append("Set ").append(vs).append(": ").append(quantity)
						.append(" ").append(unit);
			}

			break;
		case "CallSubmodule":
			details.append("Call submodule ").append(
					state.get("submodule").getAsString());
			break;
		case "MultiObservation":
		case "DiagnosticReport":
			details.append("Group the last ")
					.append(state.get("number_of_observations").getAsString())
		...
		if (state.has("codes")) {
			...
			codes.forEach(c -> {
				...
				String code = coding.get("code").getAsString();
				String display = coding.get("display").getAsString();
				details.append(system).append("[").append(code).append("]: ")
						.append(display).append(NEWLINE);
		...
		if (state.has("target_encounter")) {
			...
			details.append(verb).append(" at ")
					.append(state.get("target_encounter").getAsString())
					.append(NEWLINE);
		}
		if (state.has("reason")) {
			details.append("Reason: ")
					.append(state.get("reason").getAsString()).append(NEWLINE);
		}
		if (state.has("medication_order")) {
			details.append("Prescribed at: ")
					.append(state.get("medication_order").getAsString())
					.append(NEWLINE);
		}
		if (state.has("condition_onset")) {
			details.append("Onset at: ")
					.append(state.get("condition_onset").getAsString())
					.append(NEWLINE);
		}
		if (state.has("allergy_onset")) {
			details.append("Onset at: ")
					.append(state.get("allergy_onset").getAsString())
					.append(NEWLINE);
		}
		if (state.has("careplan")) {
			details.append("Prescribed at: ")
					.append(state.get("careplan").getAsString())
					.append(NEWLINE);
		}
		if (state.has("assign_to_attribute")) {
			details.append("Assign to Attribute: ")
					.append(state.get("assign_to_attribute").getAsString())
					.append(NEWLINE);
		}
		if (state.has("referenced_by_attribute")) {
			details.append("Referenced By Attribute: ")
					.append(state.get("referenced_by_attribute").getAsString())
					.append(NEWLINE);
		}
					...
					.forEach(
							a -> {
								...
								String display = coding.get("display")
										.getAsString();
								details.append(system).append("[").append(code)
										.append("]: ").append(display)
										.append(NEWLINE);
							});
		}
			...
			state.get("goals")
					...
					.forEach(
							gl -> {
								...
								if (goal.has("text")) {
									details.append(
											goal.get("text").getAsString())
											.append(NEWLINE);
								} else if (goal.has("codes")) {
									JsonObject coding = goal.get("codes")
									...
									String display = coding.get("display")
											.getAsString();
									details.append(system).append("[")
											.append(code).append("]: ")
											.append(display).append(NEWLINE);
								} else if (goal.has("observation")) {
									JsonObject logic = goal.get("observation")
											.getAsJsonObject();
									String obs = findReferencedType(logic);
									details.append("Observation ")
											.append(obs)
											.append("\\")
											.append(logic.get("operator")
													.getAsString())
											.append(" ")
											.append(logic.get("value")
													.getAsString())
											.append(NEWLINE);
								}
							});
		...
		if (state.has("duration")) {
			...
			details.append(NEWLINE).append("Duration: ").append(low)
					.append(" - ").append(high).append(" ").append(unit)
					.append(NEWLINE);
		}
		if (state.has("category")) {
			details.append("Category: ").append(state.get("category"))
					.append(NEWLINE);
		}
	...
	private static String logicDetails(JsonObject logic)
	{
		...
		case "Or":
			...
			{	
				...
				if (innerConditionType.equals("And")
						|| innerConditionType.equals("Or")) 
				{
		...
		case "Age":
			return "age \\" + logic.get("operator").getAsString() + " "
					+ logic.get("quantity").getAsString() + " "
					+ logic.get("unit").getAsString() + NEWLINE;
		case "Socioeconomic Status":
			return logic.get("category").getAsString()
					+ " Socioeconomic Status" + NEWLINE;
		case "Race":
		...
		case "PriorState":
			if (logic.has("within")) {
				...
				return "state '" + logic.get("name").getAsString()
						+ "' has been processed within "
						+ within.get("quantity").getAsString() + " "
						+ within.get("unit").getAsString() + NEWLINE;
			} else {
				return "state '" + logic.get("name").getAsString()
						+ "' has been processed" + NEWLINE;
			}
		case "Attribute":
			String value = logic.has("value") ? logic.get("value")
					.getAsString() : "";
			return "Attribute: " + logic.get("attribute").getAsString() + " \\"
					+ logic.get("operator").getAsString() + " " + value
					+ NEWLINE;
		case "Observation":
			String obs = findReferencedType(logic);
	...
	private static String findReferencedType(JsonObject logic)
	{
		...
		{
			String conditionType = logic.get("condition_type").getAsString();
			throw new RuntimeException(conditionType + " condition myst be specified by code or attribute");
		}
	}
}
