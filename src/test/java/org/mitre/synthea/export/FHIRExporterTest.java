...
public class FHIRExporterTest {
					...
					if(eresult.isSuccessful() == false) {
						for(SingleValidationMessage emessage : eresult.getMessages()) {
							    ...
							    || emessage.getSeverity() == ResultSeverityEnum.FATAL)
							{
								boolean valid = false;
								/*
								 * There are a few bugs in the FHIR schematron files that are distributed
								 * with HAPI 3.0.0 (these are fixed in the latest `master` branch), specifically
								 * with XPath expressions.
								 *
								 * Two of these bugs are related to the FHIR Invariant rules obs-7 and con-4,
								 * which have XPath expressions that incorrectly raise errors on validation.
								 */
								if(emessage.getMessage().contains("Message=obs-7")) {
								  /* The obs-7 invariant basically says that Observations should have values,
								   * unless they are made of components. This test replaces an invalid XPath
								   * expression that was causing correct instances to fail validation. */
	...
	/**
	 * The obs-7 invariant basically says that Observations should have values, unless they
	 * are made of components. This test replaces an invalid XPath expression that was causing
	 * correct instances to fail validation.
   ...
   * resolved, or remission" which is very clear and sensical. However, the XPath expression
   * does not evaluate correctly for valid instances, so we must manually validate.
	 * @param con
	 * @return
	 */
