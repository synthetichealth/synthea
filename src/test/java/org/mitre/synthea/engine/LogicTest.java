...
public class LogicTest {
	...
	private void setPatientAge(int age) {
		LocalDateTime now = LocalDateTime.ofInstant(Instant.ofEpochMilli(time),
				ZoneId.of("UTC"));
	...
	public void test_condition_condition() {
		...
		HealthRecord.Code alzCode = new HealthRecord.Code("SNOMED-CT",
				"26929004", "Alzheimer's disease (disorder)");

		HealthRecord.Entry cond = person.record.conditionStart(time,
				alzCode.code);
