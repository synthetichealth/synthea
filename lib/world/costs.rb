...
module Synthea
  class Costs
    ...
    def self.read_from_addendum(input_file,is_rvu)
      ...
      if is_rvu
        ...
        for entry in hashed_data
          formattedData[entry[:cpt1hcpcs].to_s] = entry
        end
        File.open(File.join(File.dirname(__FILE__), '..', '..', 'resources', 'relative_value_units.json'), 'w') do |f|
          
          content = JSON.pretty_generate(formattedData).gsub("},","},\n")
    ...
    def self.get_row_by_value(data,value,is_rvu)
      ...
      @@cached_row = data.select{ |row| row[symbol] == value } if !is_rvu
    end
...
=begin
   #test
   rvu_data = read_from_addendum($rvu_file)
   gpci_data = read_from_addendum($gpci_file)
   print medicare_allowable_payment(rvu_data,'95044',gpci_data,'REST OF MASSACHUSETTS',true)
=end
  end
end
